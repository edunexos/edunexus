<table class="w-full relative divide-gray-200" x-data="{ selected: @entangle('selected') }">
    <thead class="border-b border-neutral-200">
        <tr class="group">
            @if($this->canSelect())
                <th class="p-0 text-left text-black bg-white">
        ...
        <tr class="group">
            @if($this->canSelect())
                <th class="p-0 text-left text-black bg-white"></th>
            @endif
            @foreach($table['columns'] as $column)
                @continue(! in_array($column->code(), $this->columns))
                <th class="p-0 text-left text-black bg-white">
    ...
    <tbody>
        @if($this->deferLoading && ! $this->initialized)
            <tr class="group">
                <td class="p-0" colspan="{{ $table['columns']->count() + 1 }}">
                    <span class="block text-lg text-center py-20 bg-white text-black">
        ...
        @else
                ...
                >
                    @if($this->canSelect())
                        <td class="p-0 text-black"
                            x-bind:class="~selected.indexOf('{{ $item->getKey() }}')
                                    ? 'bg-blue-100 group-odd:bg-blue-100 group-hover:bg-blue-200'
                                    : 'bg-neutral-100 group-odd:bg-white group-hover:bg-neutral-200'">
                            <div class="mx-3">
                    ...
                    @foreach($table['columns'] as $column)
                        ...
                        <td class="p-0 text-black"
                            ...
                            @endif
                            x-bind:class="~selected.indexOf('{{ $item->getKey() }}')
                                    ? 'select-none cursor-pointer bg-blue-100 group-odd:bg-blue-100 group-hover:bg-blue-200'
                                    : 'select-none cursor-pointer bg-neutral-100 group-odd:bg-white group-hover:bg-neutral-200'">
                            {{ $column->render($item) }}
                        </td>
                    @endforeach
                </tr>
            @empty
                <tr class="group">
                    <td class="p-0 text-black" colspan="{{ $table['columns']->count() + 1 }}">
                        <span class="block text-lg text-center py-20 bg-white text-black">
                            @lang('No results')
                        </span>
                    </td>
